...
pub const SocketPair = struct {
    ...
    pub fn init(opts: Opts) SocketPair {
        var address = posix_shim.Address.parseIp("127.0.0.1", opts.port orelse 0) catch unreachable;
        var address_len = address.getOsSockLen();

        const listener = posix_shim.socket(address.any.family, posix.SOCK.STREAM | posix.SOCK.CLOEXEC, posix.IPPROTO.TCP) catch unreachable;
        defer posix_shim.close(listener);

        {
            // setup our listener
            posix_shim.bind(listener, &address.any, address_len) catch unreachable;
            posix_shim.listen(listener, 1) catch unreachable;
            posix_shim.getsockname(listener, &address.any, &address_len) catch unreachable;
        }

        const client = posix_shim.socket(address.any.family, posix.SOCK.STREAM, posix.IPPROTO.TCP) catch unreachable;
        {
            // connect the client
            const flags = posix_shim.fcntl(client, posix.F.GETFL, 0) catch unreachable;
            _ = posix_shim.fcntl(client, posix.F.SETFL, flags | posix.SOCK.NONBLOCK) catch unreachable;
            posix_shim.connect(client, &address.any, address_len) catch |err| switch (err) {
                error.WouldBlock => {},
                else => unreachable,
            };
            _ = posix_shim.fcntl(client, posix.F.SETFL, flags) catch unreachable;
        }

        const server = posix_shim.accept(listener, &address.any, &address_len, posix.SOCK.CLOEXEC) catch unreachable;
