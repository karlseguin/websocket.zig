...
pub fn ConnManager(comptime H: type, comptime MANAGE_HS: bool) type {
    return struct {
        ...
        pub fn cleanup(self: *Self, hc: *HandlerConn(H)) void {
            ...
            //     c.writer.deinit(); // TODO: Fix for Zig 0.14.0
            // }
        ...
        fn setupCompression(self: *Self, hc: *HandlerConn(H)) !void {
            ...
            compression.* = .{
                ...
                // .writer = std.io.Writer.Allocating.init(self.allocator), // TODO: Fix for Zig 0.14.0
            };
...
pub const Conn = struct {
    ...
    pub fn writeFrame(self: *Conn, op_code: OpCode, data: []const u8) !void {
        ...
        //     } else {
        //         c.writer.deinit();
        //         // c.writer = std.io.Writer.Allocating.init(c.allocator); // TODO: Fix for Zig 0.14.0
        //     }
        // };
    ...
    pub const Writer = struct {
        ...
        // interface: std.io.Writer, // TODO: Fix for Zig 0.14.0
...
const TestHandler = struct {
    ...
    pub fn clientMessage(
        ...
        if (std.mem.eql(u8, data, "writer")) {
            ...
            try wb.buf.writer().print("{d}!!!", .{9000});
            return wb.send();
        }
