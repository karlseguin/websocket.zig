...
fn ConnManager(comptime H: type) type {
	return struct {
		...
		fn create(self: *Self, socket: posix.socket_t, address: net.Address) !*HandlerConn(H) {
			errdefer posix.close(socket);
